...
export default function Camera({ artifact, onImageCaptured }: CameraProps) {
	const [imagePath, setImagePath] = useState(
		`/images/${artifact.exhibit.toLowerCase()}/${artifact.id}-unfound.png`
	);
	...
	return (
		<div className="relative flex flex-col items-center justify-center w-screen h-screen bg-black">
			...
			<div className="absolute top-5 flex flex-row w-[40svh] h-[10svh] justify-between items-center">
				...
				<button
					onClick={toggleHint}
					className={`absolute top-4 right-4 rounded-full p-3 shadow-lg z-10 flex items-center gap-2 border-none ${
						dialogOpen
							? "bg-white text-[#3e65c8]"
							: "bg-transparent text-[#3e65c8]"
					}`}
					aria-label="Hint"
				>
					{dialogOpen ? (
