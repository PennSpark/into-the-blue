...
const ImageCutout = dynamic(() => import("@/utils/ProcessSticker"), { ssr: false });
...
export default function Camera({ artifact, onImageCaptured }: CameraProps) {
  ...
  useEffect(() => {
    const globalErrorHandler = (event: ErrorEvent) => {
      if (event.message && event.message.includes("BlobURLs are not yet supported")) {
        ...
        // Activate your private mode warning modal
        setShowPrivateModeWarning(true);
        // Optionally disable further processing if needed
  ...
  return (
    ...
    >
      {/* Private Browsing Warning - kept but not automatically triggered */}
